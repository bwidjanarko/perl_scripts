...
my %count_per_hour = ();
my %count_per_day = ();
my $output_file = "monthly_report_" . $date_time . ".txt";
my $output_file1 = "monthly_report1_" . $date_time . ".txt";
...
print OUTPUT_FILE1 "hours|call_attempts|total_voice|rejected|answered|abandoned|no_answer|busy|network_sms_failure|network_failure|network_failure1|exception|uncharged|charge_success|total_sms|sms_success|sms_failure|voice_sr";
$, = '|';
...
foreach my $hour (sort(keys %count_per_hour)) {
        ...
        if ($cha > 0) {
           ...
           #print $hour, $datetime1;
              ...
              print OUTPUT_FILE1 $hour,$ca,$tv,$re,$ans,$aba,$noa,$bus,$nsf,$nf,$nf1,$ex,$un,$cs,$tsms,$smss,$smsf,,sprintf("%.5f", $voice_sr);
           #}
        #}
}
close(OUTPUT_FILE);
