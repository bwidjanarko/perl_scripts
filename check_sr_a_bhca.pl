...
my $filename1 = "/home/cdr/pull/a1/mics.cdr." . get_month_5_day_ago() . "*";
my $filename2 = "/home/cdr/pull/a2/mics.cdr." . get_month_5_day_ago() . "*";
my $filename3 = "/home/cdr/pull/a1/mics.cdr.2019-01-18*";
my $filename4 = "/home/cdr/pull/a2/mics.cdr.2019-01-18*";
my $filename5 = "/home/cdr/pull/a1/mics.cdr.2019-02*";
my $filename6 = "/home/cdr/pull/a2/mics.cdr.2019-02*";
my (@files) = `ls $filename5 $filename6`;
my $total_cdr = 0;
...
my %count_per_hour = ();
my %count_per_day = ();
my $output_file = "bhca_report_a_" . $date_time . ".txt";
my $output_file1 = "bhca_report1_a_" . $date_time . ".txt";
my $output_file2 = "bhca_report2_a_" . $date_time . ".txt";
...
print OUTPUT_FILE "hours|call_attempts|total_voice|rejected|answered|abandoned|no_answer|busy|other|network_failure|exception|uncharged|charging_success|charging_failure|total_sms|sms_success|sms_failure|call_sr|voice_sr|charging_sr|gsm|sip";
print OUTPUT_FILE1 "hours|call_attempts|total_voice|rejected|answered|abandoned|no_answer|busy|network_sms_failure|network_failure|network_failure1|exception|uncharged|charge_success|total_sms|sms_success|sms_failure|voice_sr|gsm|sip";
$, = '|';
...
foreach my $hour (sort(keys %count_per_hour)) {
        ...
        my $date = substr $hour, 0, 8;
        my $hour_only = substr $hour, 8, 2;
        unless (exists $count_per_day{$date}) {
           $count_per_day{$date}{'TotalAttempt'} = 0;
           $count_per_day{$date}{'TotalVoice'} = 0;
           $count_per_day{$date}{'TotalGSM'} = 0;
           $count_per_day{$date}{'TotalSIP'} = 0;
           $count_per_day{$date}{'PeakAttempt'} = 0;
           $count_per_day{$date}{'PeakVoice'} = 0;
           $count_per_day{$date}{'PeakGSM'} = 0;
           $count_per_day{$date}{'PeakSIP'} = 0;
           $count_per_day{$date}{'PeakHour'} = "";
        }
        $count_per_day{$date}{'TotalAttempt'} += scalar($ca);
        $count_per_day{$date}{'TotalVoice'} += scalar($tv); 
        $count_per_day{$date}{'TotalGSM'} += scalar($gsm);
        $count_per_day{$date}{'TotalSIP'} += scalar($sip);
        my $peakAttempt = $count_per_day{$date}{'PeakAttempt'};
        if (scalar($ca) > $peakAttempt) {
           $count_per_day{$date}{'PeakAttempt'} = scalar($ca);
           $count_per_day{$date}{'PeakVoice'} = scalar($tv);
           $count_per_day{$date}{'PeakGSM'} = scalar($gsm);
           $count_per_day{$date}{'PeakSIP'} = scalar($sip);
           $count_per_day{$date}{'PeakHour'} = $hour_only;
        }
...
print OUTPUT_FILE2 "date|total_attempt|total_voice|total_gsm|total_sip|bhca|bhca_voice|bhca_gsm|bhca_sip|peak_hour";
foreach my $date (sort(keys %count_per_day)) {
        my $ca = $count_per_day{$date}{'TotalAttempt'};
        my $tv = $count_per_day{$date}{'TotalVoice'};
        my $gsm = $count_per_day{$date}{'TotalGSM'};
        my $sip = $count_per_day{$date}{'TotalSIP'}; 
        my $pca = $count_per_day{$date}{'PeakAttempt'};
        my $ptv = $count_per_day{$date}{'PeakVoice'};
        my $pgsm = $count_per_day{$date}{'PeakGSM'};
        my $psip = $count_per_day{$date}{'PeakSIP'};
        my $phour = $count_per_day{$date}{'PeakHour'};
        print OUTPUT_FILE2 $date,$ca,$tv,$gsm,$sip,$pca,$ptv,$pgsm,$psip,$phour;
}
close(OUTPUT_FILE2);
